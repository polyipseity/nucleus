...
- `src/scripts/` contains Nix-internal scripts organized into domain subdirectories:
  ...
  - `apps/` — nucleus apps scripts (app lifecycle, app metadata, app bundling helpers)
  - **Naming rule**:
    ...
    - All other subdirs (`services/`, `configs/`, `packages/`, `editors/`, `secrets/`, `agents/`, `apps/`): use natural words/phrases; entry name = filename.
  ...
  - **Placement policy for `hosts/`**:
    ...
    2. **Cross-platform scripts → never in `hosts/`.** If a script is cross-platform
       ...
       `packages/`, `editors/`, `lib/`, `secrets/`, `shell/`, `agents/`, `apps/`, or the root
       of `src/scripts/`).

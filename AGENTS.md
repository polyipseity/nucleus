...
- The root `.gitignore` is a hard invariant: never edit it, stage its changes, or remove entries that look stale — even when a path it ignores no longer exists. Escalate any perceived need to the user instead. Generated gitignore files (e.g. `src/modules/configs/git/system.gitignore`) are hook-owned and must not be manually edited or committed. Other `.gitignore` files require explicit user request. Untracked build pollution (e.g. `node_modules/` from `bun install`) must be removed immediately, never silenced with gitignore edits.

...
- Use single-file modules only in `src/modules/`. Do not create `src/modules/<name>/` directories. The only allowed exception is `src/modules/macos/` (daemon-refresh.nix, finder-sidebar.nix, preference-gc.nix).

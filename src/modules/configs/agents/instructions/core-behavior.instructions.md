...
- **Pick the most specific abstract type for the capabilities needed.** Do not use a general type like `Collection` when you need ordered access — use `Sequence`/`Seq` instead. Do not use `Collection` when set membership speed (O(1) lookup) would improve performance or when uniqueness semantics matter — use `Set`/`MutableSet` instead. Choose the narrowest type that supports the operations actually required by the code.
- **Accept abstract, return concrete.** In function signatures, use abstract types for parameters (the caller may pass any compatible value). Return concrete types for outputs (the caller knows exactly what they get and can rely on specific capabilities). Exceptions: protocols and abstract factories may return abstract types.
...
- **Prefer structural typing over nominal inheritance for interfaces.** Use protocols/interfaces/typeclasses to define type contracts based on the operations an object supports, rather than requiring a specific base class. Reserve class inheritance for implementation sharing, not interface definition.
- **Fully parameterize generic types.** Never leave generic types unparameterized (`list` instead of `list[str]`, `dict` instead of `dict[str, int]`, `Callable` instead of `Callable[[int], str]`). A bare generic is as vague as `Any` — it discards all type information about elements, keys, values, and signatures.

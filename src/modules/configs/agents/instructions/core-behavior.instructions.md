...
- **No fallbacks in activation scripts.** Activation scripts must not include fallback paths to nixpkgs alternatives. If a runtime prerequisite is absent, the script should report the skip/failure, not fall back to a different implementation. Fallbacks hide real problems and add unnecessary complexity. If you find yourself adding a fallback, reconsider whether the activation script should be doing this at all.

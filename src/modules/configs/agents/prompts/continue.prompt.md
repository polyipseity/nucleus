...
- **First, check for an in-progress implementation plan:**
  ...
  2. If the file exists, read it — it contains the active plan with a lifecycle frontmatter.
  3. Parse the frontmatter to recover input variables (`atomicCommits`, `backwardsCompat`, `maxConcurrency`) and current progress (`status`, `current-step`). Re-apply these to the resumed execution (e.g., commit atomically if `atomicCommits: yes`).

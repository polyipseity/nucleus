      ...
      lib.hm.dag.entryAfter
        ...
        ''
          ...
          resolve_submodule_branch() {
            ...
            # "follow remote HEAD".
            # WHY || true: git config --get exits 1 when the key does not exist.
            branchName="$(cd "$repoPath" && git config --file .gitmodules --get "submodule.$submoduleName.branch" || true)"
            if [ "$branchName" = "." ] || [ -z "$branchName" ]; then
              # WHY || true: remote HEAD may not exist for detached submodules;
              # git symbolic-ref exits 1 on detached HEAD (fallback below extracts branch name).

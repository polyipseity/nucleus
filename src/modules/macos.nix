...
lib.mkIf pkgs.stdenv.isDarwin {
  ...
  home.activation = {
    ...
    # -------------------------------------------------------------------------
    # configureFinderSidebar
    # Finder favorites are intentionally NOT managed declaratively through
    # NSKeyedArchiver edits or sfltool-style mutations on modern macOS.
    #
    # WHY: upstream and community references (including nix-darwin #1663 and
    # linked discussions) show no stable public API for Favorites automation on
    # Ventura/Sonoma/Sequoia. Previous archive rewrites produced broken "?"
    # entries and non-functional favorites.
    #
    # Current strategy:
    #   1. Keep canonical sidebar target paths present.
    #   2. Perform a one-time repair migration that removes known-bad legacy
    #      shared-file-list state generated by previous automation attempts.
    #   3. Let Finder own the sidebar after reset; users add favorites manually.
    configureFinderSidebar = lib.hm.dag.entryAfter [ "writeBoundary" ] ''
      ...
      _finder_state_dir="$HOME/.local/state/nucleus"
      _finder_repair_marker="$_finder_state_dir/finder-sidebar-repair-v1.done"
      _finder_sfl_root="$HOME/Library/Application Support/com.apple.sharedfilelist"
      _finder_sidebar_plist="$HOME/Library/Preferences/com.apple.sidebarlists.plist"

      if [ ! -f "$_finder_repair_marker" ]; then
        mkdir -p "$_finder_state_dir"

        _finder_backup_dir="$_finder_state_dir/finder-sidebar-backup-$(date +%Y%m%d%H%M%S)"
        mkdir -p "$_finder_backup_dir"

        # Stop Finder before manipulating sidebar state files to avoid cache
        # races and stale file handles.
        if ! /usr/bin/killall Finder; then
          echo "macos: Finder was not running before sidebar repair migration." >&2
        fi

        # Back up and remove known shared-file-list payloads that can retain
        # broken entries from old automation.
        for _finder_sfl_file in \
          "$_finder_sfl_root/com.apple.LSSharedFileList.FavoriteItems.sfl" \
          "$_finder_sfl_root/com.apple.LSSharedFileList.FavoriteItems.sfl2" \
          "$_finder_sfl_root/com.apple.LSSharedFileList.FavoriteItems.sfl3" \
          "$_finder_sfl_root/com.apple.LSSharedFileList.FavoriteItems.sfl4"
        do
          if [ -f "$_finder_sfl_file" ]; then
            cp "$_finder_sfl_file" "$_finder_backup_dir/"
            rm "$_finder_sfl_file"
          fi
        done

        if [ -f "$_finder_sidebar_plist" ]; then
          cp "$_finder_sidebar_plist" "$_finder_backup_dir/"
          rm "$_finder_sidebar_plist"
        fi

        touch "$_finder_repair_marker"
        echo "macos: Finder sidebar repair migration complete; backup saved to $_finder_backup_dir" >&2
        echo "macos: add favorites manually in Finder using: ~/dev, ~/clouds, ~/clouds/GoogleDrive, ~/clouds/iCloud, ~/clouds/OneDrive" >&2
      fi
    '';

...
ensure_prek_hooks_installed() {
  ...
  # Skip noisy reinstalls when the repo already has the canonical prek hook
  # shims for the default hook set declared in prek.toml.
  if [ -f "$_ephi_hook_dir/commit-msg" ] &&
     [ -f "$_ephi_hook_dir/pre-commit" ] &&
     [ -f "$_ephi_hook_dir/pre-push" ] &&
     grep -qF '# File generated by prek' "$_ephi_hook_dir/commit-msg" &&
     grep -qF '# File generated by prek' "$_ephi_hook_dir/pre-commit" &&
     grep -qF '# File generated by prek' "$_ephi_hook_dir/pre-push"; then
    return
  fi

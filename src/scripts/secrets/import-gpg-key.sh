...
if [ "${#_gpg_secret_paths[@]}" -eq 0 ]; then
  echo "gpg-import: no GPG secret paths provided; nothing to import." >&2
  exit 1
fi
...
for _gpg_secret_path in "${_gpg_secret_paths[@]}"; do
  ...
  if [ -z "$first_key_fingerprint" ]; then
    echo "secrets: could not determine managed primary fingerprint for '$_gpg_secret_path' before import." >&2
    exit 1
  fi
...
for _gpg_secret_path in "${_gpg_secret_paths[@]}"; do
  if ! "$_gpg_bin" --import "$_gpg_secret_path"; then
    echo "secrets: gpg import failed for GPG secret '$_gpg_secret_path'." >&2
    exit 1
  fi
done
...
for current_fpr in "${current_fingerprints[@]}"; do
  if ! printf '%s:6:\n' "$current_fpr" | "$_gpg_bin" --import-ownertrust; then
    echo "secrets: warning — failed to enforce ultimate ownertrust for managed primary fingerprint $current_fpr; key is imported and tracked but trust state may require manual repair." >&2
  fi
done

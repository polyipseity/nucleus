...
if [ ! -f '__GIT_SECRET_PATH__' ]; then
  echo "git-identity: missing decrypted Git identity secret at $GIT_SECRET_PATH." >&2
  exit 1
fi

identity_name="$(/usr/bin/grep -m1 '^name=' '__GIT_SECRET_PATH__' | /usr/bin/cut -d '=' -f 2-)"
identity_email="$(/usr/bin/grep -m1 '^email=' '__GIT_SECRET_PATH__' | /usr/bin/cut -d '=' -f 2-)"
identity_signing_key="$(/usr/bin/grep -m1 '^signingKey=' '__GIT_SECRET_PATH__' | /usr/bin/cut -d '=' -f 2-)"
...
'__GIT_BIN__' config --file "$identity_file" user.name "$identity_name"
'__GIT_BIN__' config --file "$identity_file" user.email "$identity_email"
'__GIT_BIN__' config --file "$identity_file" user.signingkey "$identity_signing_key"

...
_nucleus_resolve_console_user() {
  _nucleus_console_uid="$(/usr/bin/stat -f%u /dev/console 2>/dev/null || true)" # undoc-supp: /dev/console inaccessible in headless/SSH session; handled by the empty check below
  _nucleus_console_user="$(/usr/bin/stat -f%Su /dev/console 2>/dev/null || true)" # undoc-supp: /dev/console inaccessible in headless/SSH session; handled by the empty check below

...
function Cache-FileLists {
  $script:CachedNixFiles = Get-ChildItem -Recurse -Filter '*.nix' | Where-Object { $_.FullName -notmatch '[/\]vendor[/\]' } | Sort-Object Name  # ref: exclude-lists.instructions.md#B7 — reason: structural invariant
  $script:CachedYamlFiles = Get-ChildItem -Recurse -Include '*.yml', '*.yaml' | Where-Object { $_.FullName -notmatch '[/\]vendor[/\]' } | Sort-Object Name  # ref: exclude-lists.instructions.md#B7 — reason: structural invariant
  $script:CachedJsonFiles = Get-ChildItem -Path 'src' -Recurse -Filter '*.json' | Where-Object { $_.Name -notmatch '\.schema\.json$' -and $_.FullName -notmatch '[/\]vendor[/\]' } | Sort-Object Name  # ref: exclude-lists.instructions.md#A7,#B7 — reason: schema files are meta; vendor is structural invariant

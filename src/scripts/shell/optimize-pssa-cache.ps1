...
.DESCRIPTION
  ...
  2. Reflects into the internal cache and inserts dummy RemoteCommandInfo
     entries for every command name found in the target files.
  3. Entries are inserted for both key types (74 and 383) that AvoidAlias uses.
...
.EXAMPLE
  $count = Initialize-PSScriptAnalyzerCache -Files $fileList -SettingsFile $settingsPath

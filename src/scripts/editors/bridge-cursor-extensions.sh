#!/usr/bin/env bash
# Cursor extension bridge activation.
# Called by home-manager activation symlink-cursor-extensions.
...
# setup_cursor_extension_dir DIR
# Verify DIR is a real writable directory containing per-extension symlinks
# into the Nix-managed source tree.  Cursor writes extensions.json inside
# this directory; a whole-directory symlink to the immutable Nix store
# prevents that with EACCES.
#
# Algorithm (identical to VS Code bridge):
#   1. Fail if DIR is a symlink (fix manually per MANUAL.md).
...
#      .obsolete (Cursor deferred-deletion dotfile).
#   4. Remove extensions.json so Cursor rescans the directory on next
#      invocation.  When absent, Cursor derives the manifest from the
...
setup_cursor_extension_dir() {
  ...
  # .obsolete is a Cursor deferred-deletion marker written as a dotfile
  ...
  # Step 4: remove extensions.json so Cursor rescans the directory on
  # next invocation and derives a fresh manifest from the symlink set.
  # Cursor creates this file from a directory scan when absent; when the
  # file is present Cursor trusts it and skips the scan, so a stale file

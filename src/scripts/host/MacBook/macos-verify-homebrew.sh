    ...
    if [ -n "$hb_repo_root" ] && [ -f "$hb_repo_root/src/scripts/host/MacBook/macos-homebrew-pin-verify.sh" ]; then
      NUCLEUS_REPO_ROOT="$hb_repo_root" sh "$hb_repo_root/src/scripts/host/MacBook/macos-homebrew-pin-verify.sh" || true  # undoc-supp: warning-only version check; must not abort activation even if script errors.
    fi

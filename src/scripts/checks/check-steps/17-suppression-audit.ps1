Register-Step -Id "suppression-audit" -Number 17 -Name "Suppression audit" -Action {
  ...
  if ($HasArgs) {
    ...
    $ps1Files = @($ps1Files | Where-Object { $_ -notmatch '17-suppression-audit\.ps1$' })  # ref: allow-and-deny-lists.instructions.md#A9 -- self-reference: scan definitions contain the literal suppression patterns being detected
    ...
    $allPs1 = @(
      Get-ChildItem -Recurse -Path $r -Include '*.ps1' |
        Where-Object { $_.FullName -notmatch '[\/]vendor[\/]' } |  # ref: allow-and-deny-lists.instructions.md#B5 -- structural invariant
        Where-Object { $_.Name -notmatch '^17-suppression-audit\.ps1$' } |  # ref: allow-and-deny-lists.instructions.md#A9 -- self-reference: scan definitions contain the literal suppression patterns being detected
        ForEach-Object { $_.FullName }
    )

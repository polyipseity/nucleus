Register-Step -Id "code-formatting" -Number 1 -Name "Code formatting and linting (treefmt equivalent)" -Action {
  ...
  if ($HasArgs) {
    ...
    $yamlFiles = if ($script:CachedYamlFiles) {
      ...
      Get-ChildItem -Recurse -Path $r -Include '*.yml', '*.yaml' |
        Where-Object { $_.FullName -notmatch '[/\\]vendor[/\\]' -and $_.FullName -notmatch '[/\\]secrets[/\\]' } |  # ref: allow-and-deny-lists.instructions.md#B2 -- structural invariants

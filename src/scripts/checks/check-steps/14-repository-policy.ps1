Register-Step -Id "repository-policy" -Name "Repository policy" -Action {
  ...
  $pmeViolations = 0
  $pmeExcludeNames = @('check.sh', 'check.ps1', 'shell.nix', $selfLeaf, $selfShLeaf)
  ...
  if ($HasArgs) {
    ...
    $pmeGrepFiles = @(
      Get-ChildItem -Recurse -Path (Join-Path $r 'scripts'), (Join-Path $r 'src'), (Join-Path $r 'tests') `
        -Include *.sh, *.ps1, *.nix `
        -Exclude check.sh, check.ps1, shell.nix, $selfLeaf, $selfShLeaf `
  ...
  if ($HasArgs) {
    ...
    $saPs1Files = @((@(if ($Context.Ps1Files) { $Context.Ps1Files } else { $PositionalArgs | Where-Object { $_ -like '*.ps1' } })) |
      Where-Object { (Split-Path -Leaf $_) -ne $selfLeaf })
    ...
    $saAllPs1 = @(
      Get-ChildItem -Path $r -Recurse -File |
        Where-Object { $_.Extension -eq '.ps1' -and $_.FullName -notmatch '[\/]vendor[\/]' -and $_.Name -ne $selfLeaf } |
        ForEach-Object { $_.FullName }
    )

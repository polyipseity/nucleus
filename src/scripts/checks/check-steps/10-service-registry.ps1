Register-Step -Id "service-registry" -Number 10 -Name "Service registry validation" -Action {
  param($RepoRoot)
  ...
  foreach ($svcName in $svc.Keys) {
    ...
    if ($entry.ContainsKey('platforms')) {
      Write-ErrorMessage "services.json: '$svcName' uses legacy 'platforms' key — use 'hosts'"
      $svcErrors++
    }
    ...
    if (-not $entry.ContainsKey('hosts') -or $entry.hosts.Count -eq 0) {
      ...
      foreach ($hostName in $entry.hosts.Keys) {
        ...
        foreach ($flagKey in $flagKeys) {
          if ($hEntry.ContainsKey($flagKey)) {
            Write-ErrorMessage "services.json: '$svcName'.hosts.$hostName must not contain '$flagKey'"
            $svcErrors++
          }
        }
        $expectedPlatform = $registry.hosts[$hostName].platform
        if ($hEntry.platform -ne $expectedPlatform) {
          Write-ErrorMessage "services.json: '$svcName'.hosts.$hostName platform '$($hEntry.platform)' does not match registry '$expectedPlatform'"
          $svcErrors++
        }
        $type = $hEntry.type

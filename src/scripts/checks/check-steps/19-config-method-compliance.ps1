Register-Step -Number 19 -Name "Config method compliance" -Action {
  ...
  $srcFiles = Get-ChildItem -Path (Join-Path $r "src") -Recurse -Include '*.nix', '*.ps1', '*.sh' |
    Where-Object { $_.FullName -notmatch '[\/]vendor[\/]' -and $_.FullName -notmatch '[\/]configs[\/]' }  # ref: exclude-lists.instructions.md#B1 — reason: structural invariants; vendored code and config methods are different concerns

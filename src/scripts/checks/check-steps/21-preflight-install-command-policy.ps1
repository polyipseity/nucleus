Register-Step -Id "preflight-install-command-policy" -Number 21 -Name "Preflight InstallCommand policy" -Action {
  ...
  $ps1Files = if ($HasArgs) {
    ...
    @(Get-ChildItem -Recurse -Path $r -Include '*.ps1' | Where-Object { $_.FullName -notmatch '[\\/]vendor[\\/]' } | ForEach-Object { $_.FullName } | Filter-GitIgnored)  # ref: allow-and-deny-lists.instructions.md#B6 — reason: structural invariant; gitignore filter applied on top
  }

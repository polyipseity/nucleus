Register-Step -Id "package-manager-enforcement" -Number 12 -Name "Package manager usage enforcement" -Action {
  ...
  if ($HasArgs) {
    ...
    if (-not $hasShellFiles) {
      Write-Message "==== 12: Package manager usage enforcement ==== SKIPPED (no shell files to check)"
      return 2
    }
  }

Register-Step -Number 12 -Name "Locked DSC validation" -Action {
  ...
  $dscSystemFiles = Get-ChildItem (Join-Path $dscSystemDir '*.dsc.yml') | Where-Object { $_.Name -ne 'packages.dsc.yml' } | Sort-Object Name  # ref: exclude-lists.instructions.md#A3 — reason: packages DSC is generated from lockfile, not manually authored

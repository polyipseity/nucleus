...
register_step "service-registry" 10 "Service registry validation" run_10_service_registry

run_10_service_registry() {
  ...
  if [ ! -f "$_svc_json" ]; then
    ...
    # Legacy platforms key must not appear on service entries
    if jq -e '
      to_entries[]
      | select(.key | startswith("$") | not)
      | select(.value | type == "object")
      | select(.value | has("platforms"))
    ' "$_svc_json" >/dev/null 2>&1; then
      error "services.json: legacy 'platforms' key found — use 'hosts' with platform ref"
      _svc_errors=$((_svc_errors + 1))
    fi

    # Host entries must not contain flag keys (flags live on platforms only)
    if jq -e '
      to_entries[]
      | select(.key | startswith("$") | not)
    ...
    # Host platform refs must match host-platform-registry.json

...
cache_file_lists() {
  TEST_NIX_FILES=$(find tests -name '*.nix' -type f ! -name 'lib.nix' -print | filter_gitignored | sort)  # ref: allow-and-deny-lists.instructions.md#A6 — reason: test helper library excluded from namespace of test files; gitignore filter applied on top

...
if [ -d "$_csp_target_dir" ]; then
  _csp_current_rev="$(git -C "$_csp_target_dir" rev-parse HEAD 2>/dev/null || true)" # check-suppress:suppression_doc: directory may not exist yet — rev-parse failure is expected
  ...
  if ! git -C "$_csp_target_dir" fetch origin; then
    warn -l clone-superpowers "git fetch failed for $_csp_target_dir; skipping update"
    exit 0
  fi
  if ! git -C "$_csp_target_dir" checkout "$_csp_expected_rev"; then
  ...
  if ! git clone --depth 1 "$_csp_repo_url" "$_csp_target_dir"; then
    warn -l clone-superpowers "git clone failed; skipping"
    exit 0
  fi
  if ! git -C "$_csp_target_dir" fetch --depth 1 origin "$_csp_expected_rev"; then
    warn -l clone-superpowers "git fetch --depth 1 failed; skipping checkout"
    exit 0
  fi
  if ! git -C "$_csp_target_dir" checkout FETCH_HEAD; then
...
_csp_final_rev="$(git -C "$_csp_target_dir" rev-parse HEAD 2>/dev/null || true)" # check-suppress:suppression_doc: defensive fallback — warn on rev mismatch below

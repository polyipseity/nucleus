    ...
    if [ -n "$hb_repo_root" ] && [ -f "$hb_repo_root/src/scripts/secrets/verify-homebrew-unpinnable.sh" ]; then
      NUCLEUS_REPO_ROOT="$hb_repo_root" sh "$hb_repo_root/src/scripts/secrets/verify-homebrew-unpinnable.sh" || true  # undoc-supp: warning-only version check; must not abort activation even if script errors.
    fi

...
.DESCRIPTION
  Creates a batch wrapper script at %ProgramData%\nucleus\bin\ that exports
  ...
  in VS Code settings as %ProgramData%\nucleus\bin\agent-host-shell.ps1.
...
function Invoke-AgentHostShellSetup {
  ...
  .DESCRIPTION
    Writes %ProgramData%\nucleus\bin\agent-host-shell.ps1 that
  ...
  # Create wrapper directory under ProgramData (nucleus-owned, not user HOME)
  $wrapperDir = Join-Path -Path $env:ProgramData -ChildPath "nucleus\bin"
  ...
  $wrapperPath = Join-Path -Path $wrapperDir -ChildPath "agent-host-shell.ps1"

...
function Get-AndroidGuestScriptContent {
  ...
  if (-not (Test-Path -LiteralPath $path -PathType Leaf)) {
    Write-NucleusError -CommandName android-config "guest fake-wifi script not found: $path"
    exit 1
  }

  & shellcheck -x $path
  if ($LASTEXITCODE -ne 0) {
    Write-NucleusError -CommandName android-config "shellcheck failed for guest fake-wifi script: $path"
    exit 1
  }
...
function Get-AndroidMagiskApkLibDir {
  ...
  switch (Get-AndroidVmArch -VmType ([string]$Vm.type)) {
    ...
    default {
      Write-NucleusError -CommandName android-config "unsupported guest architecture for Magisk patching: $(Get-AndroidVmArch -VmType ([string]$Vm.type))"
      exit 1
    }
  }
}
...
function Invoke-AndroidEnableUsbDebugging {
  ...
  if ($LASTEXITCODE -ne 0) {
    Write-NucleusError -CommandName android-config 'failed to enable Developer options and USB debugging via settings'
    return $false
  }
...
function Invoke-AndroidRestoreRoDebuggableUser {
  ...
  if ($debuggable -eq '1') {
    Write-NucleusInfo -CommandName android-config 'repairing ro.debuggable (was 1; restoring 0 for user build)...'
    & adb -s $serial shell "su -c resetprop ro.debuggable 0"
    if ($LASTEXITCODE -ne 0) {
      Write-NucleusError -CommandName android-config 'failed to restore ro.debuggable to 0'
      return $false
    }
  }
  return $true
...
function Test-AndroidRootPropsViaSu {
  ...
  if ($rootAccess -ne '3') {
    Write-NucleusError -CommandName android-config "persist.sys.root_access is $rootAccess (expected 3) after --root"
    return $false
  }

  $debuggable = Get-AndroidSuGetprop -Vm $Vm -Name 'ro.debuggable'
  if ($debuggable -ne '0') {
    Write-NucleusError -CommandName android-config "ro.debuggable is $debuggable (expected 0) after --root; re-run --root to repair"
    return $false
  }
  return $true
}
...
function Test-AndroidDevOptionsSmoke {
  ...
  if ((($log | Out-String) -match 'failed to set system property')) {
    Write-NucleusError -CommandName android-config 'Developer options smoke test failed (Settings property write error); ro.debuggable must stay 0'
    return $false
  }
  return $true
}
...
"@
  ...
  if ($LASTEXITCODE -ne 0) {
    Write-NucleusError -CommandName android-config "failed to persist root props boot script at $servicePath"
    return $false
  }
  return $true
}
...
function Invoke-AndroidConfigRoot {
  ...
  if ($state -ne 'device') {
    Write-NucleusError -CommandName android-config "rooted debugging requires booted system (adb state device), got: $state"
    return $false
  }

  if (-not (Test-AndroidGuestHasMagiskSu -Vm $Vm)) {
    Write-NucleusError -CommandName android-config 'rooted debugging requires Magisk su; run --magisk first'
    return $false
  }
  ...
  if ($LASTEXITCODE -ne 0) {
    Write-NucleusError -CommandName android-config 'failed to apply persist.sys.root_access on guest'
    return $false
  }
  ...
  if (-not (Test-AndroidGuestHasMagiskSu -Vm $Vm)) {
    Write-NucleusError -CommandName android-config 'Magisk su not available after rooted debugging apply'
    return $false
  }

  if (-not (Test-AndroidDevOptionsSmoke -Vm $Vm)) { return $false }

  Write-NucleusInfo -CommandName android-config "rooted debugging enabled on $serial (Magisk su, persist.sys.root_access=3); next: --fake-wifi"
  return $true
}
...
function Get-AndroidBootImagePath {
  ...
  if (Test-Path -LiteralPath $img -PathType Leaf) {
    ...
    if ($cachedTag -eq $tag) {
      Write-NucleusInfo -CommandName android-config "using cached boot image: $img"
      return $img
    }
    Write-NucleusInfo -CommandName android-config "jqssun release changed ($cachedTag → $tag); re-downloading boot image..."
    Remove-Item -LiteralPath $img -Force
  }
  else {
    Write-NucleusInfo -CommandName android-config "downloading boot image ($asset)..."
  }
  ...
  try {
    ...
    Write-NucleusError -CommandName android-config "failed to download boot image from $downloadUrl"
    return $null
  }

  (@{ tag_name = $tag } | ConvertTo-Json -Compress) | Set-Content -LiteralPath $tagFile -Encoding UTF8
  Write-NucleusInfo -CommandName android-config "boot image ready: $img"
  return $img
}
...
function Get-AndroidMagiskApkPath {
  ...
  if ([string]::IsNullOrWhiteSpace($url) -or $url -eq 'null') {
    Write-NucleusError -CommandName android-config 'Android.magiskUrl is not set in the manifest'
    return $null
  }

  if (Test-Path -LiteralPath $apk -PathType Leaf) {
    Write-NucleusInfo -CommandName android-config "using cached Magisk APK: $apk"
    return $apk
  }

  Write-NucleusInfo -CommandName android-config 'downloading Magisk APK...'
  try {
    Invoke-WebRequest -Uri $url -OutFile $apk -UseBasicParsing
  }
  catch {
    Write-NucleusError -CommandName android-config "failed to download Magisk from $url"
    return $null
  }

  Write-NucleusInfo -CommandName android-config "Magisk APK ready: $apk"
  return $apk
}
...
function Expand-AndroidMagiskPatchKit {
  ...
  try {
    foreach ($entryName in $requiredEntries) {
      ...
      if ($null -eq $entry) {
        Write-NucleusError -CommandName android-config "failed to extract Magisk patch kit from $MagiskApk (missing $entryName)"
        return $false
      }
  ...
  if (-not (Test-Path -LiteralPath $bootPatch -PathType Leaf)) {
    Write-NucleusError -CommandName android-config "Magisk APK is missing assets/boot_patch.sh ($MagiskApk)"
    return $false
  }
  ...
  foreach ($pair in $pairs) {
    ...
    if (-not (Test-Path -LiteralPath $srcPath -PathType Leaf)) {
      Write-NucleusError -CommandName android-config "Magisk APK is missing lib/$libDirName/$($pair.Src)"
      return $false
    }
...
function Invoke-AndroidMagiskGuestPatchBoot {
  ...
  Write-NucleusInfo -CommandName android-config "patching boot image with Magisk on guest $serial..."
  ...
  if ($LASTEXITCODE -ne 0) {
    Write-NucleusError -CommandName android-config 'Magisk boot_patch.sh failed on guest'
    return $false
  }

  & adb -s $serial pull $remoteOut $OutImg
  if ($LASTEXITCODE -ne 0) {
    Write-NucleusError -CommandName android-config "failed to pull patched boot image from guest (expected $remoteOut)"
    return $false
  }
  ...
  Write-NucleusInfo -CommandName android-config "patched boot image: $OutImg"
  return $true
}
...
function Invoke-AndroidMagiskFlashBoot {
  ...
  Write-NucleusInfo -CommandName android-config "rebooting to fastboot on $serial (VM: Recovery → Advanced → Enter fastboot if needed)..."
  ...
  if ($LASTEXITCODE -ne 0) {
    Write-NucleusError -CommandName android-config "fastboot flash boot failed on $fbSerial"
    return $false
  }
  ...
  Write-NucleusInfo -CommandName android-config 'flashed Magisk boot image; waiting for system boot...'
  return $true
}
...
function Invoke-AndroidMagiskInstallApk {
  ...
  $serial = Get-AndroidAdbSerial -Vm $Vm
  Write-NucleusInfo -CommandName android-config "installing Magisk APK on $serial..."
  for ($attempt = 0; $attempt -lt 12; $attempt++) {
    & adb -s $serial install -r $MagiskApk
    if ($LASTEXITCODE -eq 0) { return $true }
    if ($attempt -lt 11) {
      Write-NucleusInfo -CommandName android-config 'Magisk APK install not ready yet (guest may still be booting); retrying...'
      Start-Sleep -Seconds 10
      Wait-AndroidAdbBootCompleted -Vm $Vm -TimeoutSeconds 120 > $null
    }
  }

  Write-NucleusError -CommandName android-config 'failed to install Magisk APK; tap Allow USB debugging and retry'
  return $false
}
...
function Invoke-AndroidInstallAdbKeyViaSu {
  ...
  if (-not (Test-AndroidGuestHasMagiskSu -Vm $Vm)) {
    Write-NucleusError -CommandName android-config 'Magisk su is required to install adb_keys on a booted user build'
    return $false
  }
...
function Invoke-AndroidConfigMagisk {
  ...
  $state = Get-AndroidAdbPollState -Vm $Vm
  if ($state -ne 'device') {
    Write-NucleusError -CommandName android-config "Magisk requires booted system (adb state device), got: $state"
    return $false
  }

  $build = Get-AndroidShellGetprop -Vm $Vm -Name 'ro.build.display.id'
  if ($build -match 'gsi') {
    Write-NucleusError -CommandName android-config "Magisk install targets Lineage only (detected: $build); boot Lineage, not GSI"
    return $false
  }

  if (Test-AndroidGuestHasMagiskSu -Vm $Vm) {
    Write-NucleusInfo -CommandName android-config "Magisk su is already available on $serial"
  }
  else {
    ...
    if (-not (Wait-AndroidAdbBootCompleted -Vm $Vm -TimeoutSeconds 900)) {
      Write-NucleusError -CommandName android-config 'timed out waiting for boot after Magisk flash; complete setup wizard and tap Allow USB debugging'
      return $false
    }
    if (-not (Invoke-AndroidMagiskInstallApk -Vm $Vm -MagiskApk $apk)) { return $false }
    Write-NucleusInfo -CommandName android-config 'next: open Magisk app on VM; complete environment-fix if prompted, then re-run --magisk'
    ...
    if (-not (Test-AndroidGuestHasMagiskSu -Vm $Vm)) {
      Write-NucleusError -CommandName android-config 'Magisk su not available; open Magisk app on VM, then retry --magisk'
      return $false
    }
  }
  ...
  Write-NucleusInfo -CommandName android-config "Magisk installed on $serial; next: --root"
  return $true
}
...
function Assert-AndroidFakeWifiRequireSu {
  ...
  if (Test-AndroidFakeWifiAdbShellProbe -Serial $Serial) {
    Write-NucleusError -CommandName android-config "Magisk su is not available on $Serial (adb shell works); open the Magisk app and retry"
  }
  else {
    Write-NucleusError -CommandName android-config "ADB is not reachable on $Serial; run adb disconnect && adb connect manually, then retry"
  }
  return $false
}
...
function Wait-AndroidFakeWifiAdbAfterAsync {
  ...
  Write-NucleusInfo -CommandName android-config "waiting for ADB on $Serial to recover (timeout ${TimeoutSeconds}s)..."
...
function Wait-AndroidFakeWifiReady {
  ...
  Write-NucleusInfo -CommandName android-config "waiting for guest VirtWifi setup on $Serial (ADB reconnects after join; timeout ${TimeoutSeconds}s)..."
  Start-Sleep -Seconds $script:AndroidNucleusFakeWifiAsyncGraceS
  $elapsed = $script:AndroidNucleusFakeWifiAsyncGraceS
  while ($elapsed -lt $TimeoutSeconds) {
...
function Write-AndroidFakeWifiDiagnosticReport {
  ...
  Write-NucleusInfo -CommandName android-config "fake Wi-Fi diagnostics for ${Serial}:"
  ...
  if (-not (Invoke-AndroidFakeWifiRunAsRoot -Serial $Serial -Command $diagCmd -TimeoutSeconds 30)) {
    Write-NucleusWarning -CommandName android-config 'could not collect fake Wi-Fi diagnostics (is Magisk su available?)'
  }
}
...
function Invoke-AndroidFakeWifiEnable {
  ...
  if (Test-AndroidFakeWifiWlan0Up -Serial $Serial) {
    Write-NucleusInfo -CommandName android-config "wlan0 already present on $Serial (VirtWifi already joined)"
  }
  else {
    Write-NucleusInfo -CommandName android-config "loading virt_wifi and creating wlan0 on $Serial (ADB will reconnect)..."
    if (-not (Invoke-AndroidFakeWifiRunGuestScript -Serial $Serial -ScriptContent $setupScript -Async)) {
      ...
      Write-NucleusError -CommandName android-config 'failed to start fake Wi-Fi setup on guest'
      return $false
    }
    if (-not (Wait-AndroidFakeWifiReady -Serial $Serial -TimeoutSeconds 60)) {
      Write-AndroidFakeWifiDiagnosticReport -Serial $Serial
      Write-NucleusError -CommandName android-config 'ADB or wlan0 did not become ready after fake Wi-Fi setup'
      return $false
    }
  }
...
"@
  if (Invoke-AndroidFakeWifiRunAsRoot -Serial $Serial -Command $persistCmd -TimeoutSeconds 2) {
    Write-NucleusInfo -CommandName android-config "persisted fake Wi-Fi startup at $servicePath"
  }
  else {
    Write-NucleusWarning -CommandName android-config 'could not persist fake Wi-Fi script (Magisk service.d may be unavailable); re-run after reboot if needed'
  }

  Write-NucleusInfo -CommandName android-config "fake Wi-Fi enabled on $Serial"
  return $true
}
...
function Invoke-AndroidFakeWifiRevert {
  ...
  Write-NucleusInfo -CommandName android-config "reverting fake Wi-Fi on $Serial..."
  ...
  if ($hadWlan0) {
    ...
    if (-not (Wait-AndroidFakeWifiAdbAfterAsync -Serial $Serial -TimeoutSeconds 30)) {
      Write-NucleusWarning -CommandName android-config "ADB did not recover within 30s after revert; if commands hang, run: adb disconnect $Serial && adb connect $Serial"
    }
  }
  elseif (Test-AndroidFakeWifiEth0Up -Serial $Serial) {
    Write-NucleusInfo -CommandName android-config "wlan0 already absent; eth0 is up on $Serial"
  }

  Write-NucleusInfo -CommandName android-config "fake Wi-Fi reverted on $Serial"
  return $true
}
...
function Invoke-AndroidConfigGapp {
  ...
  if ([string]::IsNullOrWhiteSpace($gappsUrl) -or $gappsUrl -eq 'null') {
    Write-NucleusError -CommandName android-config 'Android.gappsUrl is not set in the manifest'
    return $false
  }

  $state = Get-AndroidAdbPollState -Vm $Vm
  if ($state -eq 'device') {
    Write-NucleusError -CommandName android-config 'MindTheGapps requires LineageOS Recovery; guest is booted to system (device)'
    return $false
  }
  ...
  if (Wait-AndroidAdbSideload -Vm $Vm -TimeoutSeconds 15) {
    Write-NucleusInfo -CommandName android-config "guest already in sideload mode on $serial"
  }
  else {
    $state = Get-AndroidAdbPollState -Vm $Vm
    if ($state -eq 'recovery') {
      Write-NucleusInfo -CommandName android-config "entering sideload mode on $serial..."
    ...
    if (-not (Wait-AndroidAdbSideload -Vm $Vm -TimeoutSeconds 120)) {
      Write-NucleusError -CommandName android-config 'guest did not enter sideload mode; from recovery select Apply update from ADB'
      return $false
    }
  }

  if (-not (Test-Path -LiteralPath $gappsZip -PathType Leaf)) {
    Write-NucleusInfo -CommandName android-config 'downloading MindTheGapps...'
    try {
      Invoke-WebRequest -Uri $gappsUrl -OutFile $gappsZip -UseBasicParsing
    }
    catch {
      Write-NucleusError -CommandName android-config "failed to download MindTheGapps from $gappsUrl"
      return $false
    }
  }
  else {
    Write-NucleusInfo -CommandName android-config "using cached MindTheGapps: $gappsZip"
  }

  Write-NucleusInfo -CommandName android-config 'tap Install anyway on the VM if sideload asks about signature verification'
  Write-NucleusInfo -CommandName android-config "sideloading MindTheGapps to $serial..."
  & adb -s $serial sideload $gappsZip
  if ($LASTEXITCODE -ne 0) {
    Write-NucleusError -CommandName android-config 'adb sideload failed; tap Install anyway on the VM if prompted, then retry'
    return $false
  }

  Write-NucleusInfo -CommandName android-config 'MindTheGapps sideload finished'
  Write-NucleusInfo -CommandName android-config 'next: reboot system; then --magisk, --root, --fake-wifi'
  return $true
}
...
function Invoke-AndroidConfigAdbKey {
  ...
  if (-not (Test-Path -LiteralPath $pubkey -PathType Leaf)) {
    Write-NucleusError -CommandName android-config "host ADB public key not found: $pubkey (run adb once to generate keys)"
    return $false
  }
  ...
  switch ($state) {
    ...
    'unauthorized' {
      Write-NucleusError -CommandName android-config 'ADB unauthorized; enable ADB in recovery (Advanced → Enable ADB) or tap Allow on booted Lineage'
      return $false
    }
    default {
      Write-NucleusError -CommandName android-config "ADB not reachable for key install (state: $state)"
      return $false
    }
  }
  ...
  switch ($state) {
    { $_ -in @('recovery', 'sideload') } {
      if (-not (Test-AndroidGuestShellIsRoot -Vm $Vm)) {
        Write-NucleusError -CommandName android-config 'recovery adb-keys requires root shell (Advanced → Enable ADB)'
        return $false
      }
    ...
    'device' {
      if (-not (Test-AndroidGuestHasMagiskSu -Vm $Vm)) {
        Write-NucleusError -CommandName android-config 'booted adb-keys requires Magisk su (run --magisk first) or recovery --adb-keys before first boot'
        return $false
      }
      if (-not (Invoke-AndroidInstallAdbKeyViaSu -Vm $Vm -PubkeyPath $pubkey)) { return $false }
    }
    default {
      Write-NucleusError -CommandName android-config "guest must be in recovery or booted system for adb-keys; current state: $state"
      return $false
    }
  }

  Write-NucleusInfo -CommandName android-config "installed host ADB key on $serial ($state)"
...
function Invoke-AndroidConfig {
  ...
  if ($vmIndex -lt 0) {
    Write-NucleusError -CommandName android-config "VM '$VmId' not found in manifest"
    exit 1
  }

  $vm = $Manifest.VMs[$vmIndex]
  if ([string]$vm.type -ne 'Android') {
    Write-NucleusError -CommandName android-config "android-config is only supported for Android VMs ('$VmId' is type $($vm.type))"
    exit 1
  }
  ...
  foreach ($flag in $ConfigFlags) {
    switch ($flag) {
      ...
      '--adb-debug' {
        Write-NucleusError -CommandName android-config 'unknown flag: --adb-debug (did you mean --adb-keys?)'
        exit 1
      }
      '--root' { $doRoot = $true }
      ...
      default {
        Write-NucleusError -CommandName android-config "unsupported flag: $flag"
        exit 1
      }
    }
  }
  ...
  if ($doFakeWifi) {
    if (-not (Wait-AndroidAdbAuthorized -Vm $vm -TimeoutSeconds 600)) {
      Write-NucleusError -CommandName android-config 'fake Wi-Fi requires booted Lineage with authorized ADB'
      exit 1
    }
    if (-not (Test-AndroidGuestHasMagiskSu -Vm $vm)) {
      Write-NucleusError -CommandName android-config 'fake Wi-Fi requires Magisk su; run --magisk first'
      exit 1
    }
    $serial = Get-AndroidAdbSerial -Vm $vm
  ...
  if ($doFakeWifiRevert) {
    if (-not (Wait-AndroidAdbAuthorized -Vm $vm -TimeoutSeconds 600)) {
      Write-NucleusError -CommandName android-config 'fake Wi-Fi revert requires booted Lineage with authorized ADB'
      exit 1
    }
    $serial = Get-AndroidAdbSerial -Vm $vm
  ...
  Write-NucleusInfo -CommandName android-config "android-config complete for '$VmId'"
}
...
function Invoke-AndroidReset {
  ...
  $vm = $Manifest.VMs[$vmIndex]
  if ([string]$vm.type -ne 'Android') {
    Write-NucleusError -CommandName android-config "reset is only supported for Android VMs ('$VmId' is type $($vm.type))"
    exit 1
  }

  if ($vm.shareDevDir -eq $true) {
    Write-NucleusError -CommandName android-config "shareDevDir is not supported for Android VM '$VmId'; Android does not support host filesystem sharing via QEMU"
    exit 1
  }
  ...
  if (-not (Test-Path -LiteralPath $systemImg -PathType Leaf)) {
    ...
    try {
      ...
      Write-NucleusError -CommandName android-config 'failed to download LineageOS zip'
      exit 1
    }
    ...
    if ($null -eq $largestQcow2) {
      Write-NucleusError -CommandName android-config 'no qcow2 system image found inside extracted LineageOS bundle'
      exit 1
    }
    ...
    Write-NucleusInfo -CommandName android-config "system image ready: $systemImg"
  }
  else {
  ...
  if (-not (Test-Path -LiteralPath $userdataImg -PathType Leaf)) {
    Write-NucleusInfo -CommandName android-config "creating userdata disk ($diskBytes bytes)..."
    $qemuImg = Get-AndroidQemuImgPath
    if (-not $qemuImg) {
      Write-NucleusError -CommandName android-config 'qemu-img not found; install QEMU via Scoop (scoop install qemu) or add it to PATH'
      exit 1
    }
    & $qemuImg create -f qcow2 $userdataImg $diskBytes
    if ($LASTEXITCODE -ne 0) {
      Write-NucleusError -CommandName android-config "failed to create Android userdata disk: $userdataImg"
      exit 1
    }
    if (-not (Test-AndroidQcow2Image -ImagePath $userdataImg -Label "Android userdata disk for $VmId" -MinVirtualSize $minSizeBytes)) {
      exit 1
    }
    Write-NucleusInfo -CommandName android-config "userdata disk ready: $userdataImg"
  }

  # Step 3: optional GSI image
  if (-not [string]::IsNullOrWhiteSpace($gsiUrl) -and $gsiUrl -ne 'null') {
    if (-not $AcceptGsiLicense) {
      Write-NucleusError -CommandName android-config "GSI license not accepted for '$VmId'; see https://developer.android.com/license"
      exit 1
    }
    Write-NucleusInfo -CommandName android-config 'GSI license: https://developer.android.com/license'
    if (-not (Test-Path -LiteralPath $gsiImg -PathType Leaf)) {
      Write-NucleusInfo -CommandName android-config 'downloading GSI system image...'
      $gsiZip = Join-Path $androidSrcDir $script:AndroidGsiDownloadZip
      try {
        Invoke-WebRequest -Uri $gsiUrl -OutFile $gsiZip -UseBasicParsing
      }
      catch {
        Write-NucleusError -CommandName android-config "failed to download GSI zip from $gsiUrl"
        exit 1
      }
      ...
      try {
        ...
        if ($null -eq $entry) {
          Write-NucleusError -CommandName android-config 'GSI system.img not found in zip archive'
          exit 1
        }
      ...
      if (-not (Test-Path -LiteralPath $gsiImg -PathType Leaf)) {
        Write-NucleusError -CommandName android-config 'GSI system.img not found after extraction'
        exit 1
      }
      Write-NucleusInfo -CommandName android-config "GSI system image ready: $gsiImg"
    }
    else {
      Write-NucleusInfo -CommandName android-config "GSI system image already exists: $gsiImg"
    }
  }
  else {
    Write-NucleusInfo -CommandName android-config 'no GSI URL set; skipping GSI download (Lineage-only)'
  }

  Write-NucleusInfo -CommandName android-config "reset complete for '$VmId'"
}

function Sync-QtPassConfig {
  ...
  function Get-QtPassDesiredState {
    ...
    $settings = ConvertTo-Hashtable -InputObject (Get-Content -Path $settingsPath -Raw | ConvertFrom-Json)
  ...
  function Invoke-QtPassManagedCleanup {
    ...
    foreach ($settingName in $SettingNames) {
      # check-suppress:suppression_doc: each setting may already be absent from a previous cleanup pass.
  ...
  foreach ($userRecord in $Users) {
    ...
    $effectiveSettings = Get-QtPassDesiredState -Username $username -RepoRoot $RepoRoot

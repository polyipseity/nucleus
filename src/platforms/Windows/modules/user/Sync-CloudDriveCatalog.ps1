...
function Sync-CloudDriveCatalog {
    ...
    foreach ($mount in $enabledMounts) {
        ...
        #   backend owns those and composes them itself (Mount-Backend-ArgumentList).  Sending
        ...
        # WHY one assignment, and the concatenation at the wrapper build below: PSSA's
        #   PSUseDeclaredVarsMoreThanAssignments does not see a variable that is read only
        #   inside an interpolated string, so embedding $passLine in the single wrapper
        #   literal reports it as assigned-but-never-used even though the wrapper needs it.
        #   Concatenating makes the read a real read in the AST; the emitted text is
        #   byte-identical, and the pass line is the one part of the wrapper that is
        #   conditional.  Keep the concatenation: folding it back into one interpolated
        #   literal reintroduces the finding and hides the conditional from the rule.

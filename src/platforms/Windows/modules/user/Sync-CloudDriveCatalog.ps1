...
function Sync-CloudDriveCatalog {
    ...
    foreach ($mount in $enabledMounts) {
        ...
        $null = $wrapperLines.Add("# Generated by Sync-CloudDriveCatalog.ps1")  # check-suppress:suppression_doc: Add returns collection count, discarded
        $null = $wrapperLines.Add("# Task: $taskName")  # check-suppress:suppression_doc: Add returns collection count, discarded

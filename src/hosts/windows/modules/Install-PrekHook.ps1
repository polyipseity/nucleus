...
function Install-PrekHook {
  ...
  $hookPaths = @(
    (Join-Path -Path $hookDir -ChildPath "commit-msg"),
    (Join-Path -Path $hookDir -ChildPath "pre-commit"),
    (Join-Path -Path $hookDir -ChildPath "pre-push")
  )
  $hooksAlreadyInstalled = $true
  foreach ($hookPath in $hookPaths) {
    if (-not (Test-Path -Path $hookPath -PathType Leaf)) {
      $hooksAlreadyInstalled = $false
      break
    }

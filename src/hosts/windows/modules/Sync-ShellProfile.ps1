...
function Sync-ShellProfile {
  ...
  $managedBlock = @(
    ...
    '  $hookPaths = @('
    '    (Join-Path $hookDir "commit-msg"),'
    '    (Join-Path $hookDir "pre-commit"),'
    '    (Join-Path $hookDir "pre-push")'
    '  )'
    '  foreach ($hookPath in $hookPaths) {'
    '    if (-not (Test-Path -Path $hookPath -PathType Leaf)) {'
    '      return $false'
    '    }'
    '    if (-not (Select-String -Path $hookPath -Pattern ''# File generated by prek'' -SimpleMatch -Quiet)) {'
    '      return $false'
    '    }'
    '  }'
    '  return $true'

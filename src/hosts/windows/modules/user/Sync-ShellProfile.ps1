...
function Sync-ShellProfile {
  ...
  .DESCRIPTION
    ...
    Managed content intentionally mirrors key POSIX shell workflow behavior:
      ...
      - common aliases (`-g`, `-ga`, `-gc`, `-gca`, `-gco`, `-gd`, `-gl`, `-gll`, `-gp`,
  ...
  $managedBlock = @(
    ...
    '  $repoRoot = Resolve-NucleusRepoRoot'
    '  $scriptPath = Join-Path $repoRoot ''scripts\replica-sync.ps1'''
    '  if (-not (Test-Path -Path $scriptPath -PathType Leaf)) {'
    '    throw "nucleus-replica-sync: script not found at $scriptPath"'
    '  }'
    '  & $scriptPath @Args'
    '}'

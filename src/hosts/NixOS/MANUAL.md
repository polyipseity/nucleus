...
- After first install (Btrfs root per `src/hosts/NixOS/hardware/disks.nix`), generate hardware config: `sudo nixos-generate-config --dir /tmp/nixos-generate-config`. Compare with `src/hosts/NixOS/hardware/{cpu,gpu,disks}.nix` and merge host-specific facts (filesystem UUIDs, EFI `/boot`, swap, kernel modules, device paths). Uncomment the `/boot` vfat entry in `disks.nix` when merging. Rebuild to confirm no missing references. Bare-metal hosts still on ext4 require reinstall or offline conversion before applying the Btrfs template — nucleus provides no automated migration.

...
if _nucleus_resolve_console_user; then
  if [ -d "/Applications/MiddleClick.app" ]; then
    if ! /bin/launchctl asuser "$_nucleus_console_uid" /usr/bin/sudo -H -u "$_nucleus_console_user" \
      ...
      warn -l middleclick "failed to set fingers=4 for user '$_nucleus_console_user'."
    fi
    ...
    if ! /bin/launchctl asuser "$_nucleus_console_uid" /usr/bin/sudo -H -u "$_nucleus_console_user" \
      ...
      -e 'end if' \
      -e 'end tell'; then
      warn -l middleclick "failed to ensure native Login Item startup for user '$_nucleus_console_user'; enable 'Launch at login' from MiddleClick menu as fallback."
    fi
  fi
fi

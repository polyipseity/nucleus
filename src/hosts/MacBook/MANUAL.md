...
- `nucleus-vm setup` — build and provision VMs from `src/modules/VMs.json`
  ...
  - **Android guest** (LineageOS): requires UTM's global renderer backend to be **Apple Core OpenGL (CGL)** (pref `QEMURendererBackend = 3`, UTM 5.x; previously ANGLE (OpenGL), value 1), otherwise the UI never appears after boot. The pref is provisioned automatically by `nucleus-apply` (activation script `macos-set-utm-renderer.sh`), so no manual UTM settings change is needed. Known bug: the display can still freeze randomly — a client-side SPICE display-channel stall in UTM's SPICE client ([UTM #2221](https://github.com/utmapp/UTM/issues/2221), [CocoaSpice#5](https://github.com/utmapp/CocoaSpice/issues/5)), independent of the renderer backend; UTM 5.0.4 ships SPICE renderer fixes and keeping the VM window visible helps. See [LineageOS on UTM wiki](https://wiki.lineageos.org/utms/utm-vm-on-apple-silicon-mac).
    - **Google services (GSI)**: the guest ships with Google Mobile Services (GMS) via a Google GSI (`gsi_gms_arm64`), enabled by `androidGsiUrl` in `src/modules/VMs.json` — a download URL from https://developer.android.com/topic/generic-system-image/releases (e.g. `https://dl.google.com/developers/android/baklava/images/gsi/gsi_gms_arm64-exp-BP4A.251205.006-14401865-f8760221.zip`); `null` falls back to the built-in LineageOS GSI. `nucleus-apply` already passes `--accept-gsi-license` when building the VM image (the download is ~2 GB on first build).
    - **First boot**: boot the VM into recovery, factory reset, then select **Boot GSI from /dev/block/vdc** (the GSI attaches read-only as the third virtio disk; the label is the one documented for the equivalent vdc disk in the [LineageOS libvirt-qemu wiki](https://wiki.lineageos.org/utms/utm-vm-on-apple-silicon-mac) — confirm the exact UTM recovery label on first boot). This step is manual and cannot be automated from the host.

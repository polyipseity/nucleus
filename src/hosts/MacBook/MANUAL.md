...
- The 80 % charge limit uses two independent gates; `nucleus-apply` converges both. Open `battery.app` once to install `/usr/local/bin/battery`; the apply then runs `battery maintain 80`, which writes the SMC charging gate and installs the CLI's own `com.battery.app` LaunchAgent, so the cap holds without the tray being open. The app itself still starts at login through the managed `local.com.fiplab.battery` agent; what `nucleus-apply` suppresses is only its menu-bar icon. The second gate is the native **Charge Limit** (macOS 26.4+), which `nucleus-apply` drives through a Shortcuts shortcut; macOS offers no declarative way to set it (no `pmset` key, no configuration profile), so create the shortcut once by hand and name it exactly `Set Charge Limit 80`: add the **Set Battery Charge Limit** action, set it to **80 %**, save, then confirm it appears in `shortcuts list`. A missing shortcut logs a warning on each apply and leaves the `battery` gate capping charge at 80 %; a shortcut that exists but fails fails the apply. Charging needs every gate open, so the effective ceiling is the lower of the two, and both are set to 80 %. Confirm the SMC gate with `sudo /usr/local/co.palokaj.battery/smc -k CHWA -r` (`01` = engaged) and the effective state with `pmset -g batt` (`80%; AC attached; not charging`). Keep both gates: the native one holds the limit when an OS update breaks the third-party helper.

...
- Apple Command Line Tools (CLT) are not required and are removed on each `nucleus-apply` (files under `/Library/Developer/CommandLineTools` plus pkgutil receipts). Software Update should not offer CLT updates after a successful apply. A manual CLT install is undone on the next apply. If CLT update prompts persist, check `/Users/Shared/nucleus/logs/command-line-tools.log` and confirm apply completed. The Nix LLVM toolchain provides `clang`/`clang++`/`ld.lld` via absolute store paths in `CC`/`CXX`/`LD` — no PATH-based resolution occurs, so `/usr/bin/clang` (the xcrun shim) is never reached. If you see the xcrun dialog, check that `nucleus-apply` completed successfully and restart your shell session to pick up the updated environment.

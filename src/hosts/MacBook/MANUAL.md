...
- Cloud mounts require two prerequisites (replicas/sync need neither): (1) a successful `nucleus-apply` — verify with `launchctl list | grep cloud` (expect `local.cloud-mount.*` and `local.cloud-replica-scheduled-sync.*` agents), and (2) the macFUSE file-system extension enabled (installing macFUSE is not enough). To enable: System Settings → General → Login Items & Extensions → **by category** → File System Extensions → enable **macFUSE**. Registration is observable with `pluginkit -m -A -p com.apple.fskit.fsmodule`; `systemextensionsctl list` never lists FSKit modules, so it cannot verify this step. Never approve the macFUSE **kernel extension**: every mount uses the FSKit backend (`-o backend=fskit`), which needs no kernel extension. Mounts live at `/Volumes/nucleus-cloud-<id>` and `~/clouds/<id>` links to it. macFUSE creates that mount point during the first mount and assigns it to the requesting user, so do not create it by hand — a root-owned or non-empty directory there fails the mount. Run `nucleus-cloud setup` to configure remotes (`rclone config`) for GoogleDrive, iCloud, and OneDrive.
- NTFS read-write: launch Mounty, agree to the dialog, plug in an NTFS drive, then re-mount it via the Mounty menu-bar icon. If the drive mounts read-only (e.g. after Windows Fast Startup): unmount the drive, run `sudo ntfsfix /dev/diskXsY`, then reconnect the drive. Fallback: unmount and `sudo ntfs-3g -o backend=fskit /dev/diskXsY /Volumes/nucleus-ntfs`. NTFS read-write goes through the same macFUSE FSKit backend, so never approve the macFUSE kernel extension; a mount point outside `/Volumes` fails under FSKit. The ntfs-3g binary is built automatically during `nucleus-apply`. If the build fails, see `/Library/Application Support/nucleus/logs/ntfs-3g-build.log` for the full output. Mounty's menu-bar icon uses the macOS 26 Control Center `NSStatusItem Visible com.mounty.app` gate. Mounty has no ⌘-drag hide and no hide preference. The `sudo ntfs-3g` fallback above is the only no-icon path. Mounty's app-native helper (`com.cu4uc.MountyHelper`) is an embedded Service-Management helper, which macOS 13+ does not let a script unregister: turn it off in Mounty's own settings if it is registered, and keep only our registry-driven launch agent. Do not re-enable the embedded helper.

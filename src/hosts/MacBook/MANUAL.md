...
- Cloud mounts require two prerequisites (replicas/sync do not need FUSE-T): (1) a successful `nucleus-apply` — verify with `launchctl list | grep cloud` (expect `local.cloud-mount.*` and `local.cloud-replica-scheduled-sync.*` agents), and (2) the FUSE-T FSKit extension enabled (installing `fuse-t.app` is not enough). To enable: open `fuse-t.app` once, then System Settings → General → Login Items & Extensions → **by category** → File System Extensions → enable **fuse-t** (`FskitSrvModule`). Registration is observable with `pluginkit -m -p com.apple.fskit.fsmodule`; `systemextensionsctl list` never lists FSKit modules, so it cannot verify this step. Enablement itself is not verifiable from the shell, and an enabled extension can still fail: if `local.cloud-mount.*` exits 1 with `Mount failed` while `~/Library/Logs/fuse-t/fuse-t.log` reports `bind: operation not permitted` for `group.org.fuset.fskit-srv/s/*.sock`, macOS is blocking FUSE-T's FSKit socket path (observed on macOS 27) — that is not an approval problem. Run `nucleus-cloud setup` to configure remotes (`rclone config`) for GoogleDrive, iCloud, and OneDrive.

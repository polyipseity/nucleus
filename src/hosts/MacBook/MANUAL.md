...
- `nucleus-vm setup` — build and provision VMs from `src/modules/VMs.json`
  ...
  - **Android guest** (LineageOS): requires UTM's global renderer backend to be **ANGLE (OpenGL)** (UTM → Settings → Display → Renderer backend), otherwise the UI freezes or never appears after boot. See [LineageOS on UTM wiki](https://wiki.lineageos.org/utms/utm-vm-on-apple-silicon-mac) and [UTM issue #378](https://github.com/utmapp/UTM/issues/378).

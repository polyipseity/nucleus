...
- `nucleus-vm-setup` — build (if needed) and provision VMs declared in `src/modules/VMs.json`; run once per machine or when adding a VM.
  - **macOS guest** (`--macos-only` — tart): fully automatic on Apple Silicon; Packer Tart builds the VM via `vms/macos/packer.pkr.hcl`. Requires `tart` (installed via `brew install cirruslabs/cli/tart`). Start with `tart run macos [--no-graphics]`.
  - **NixOS guest** (`--nixos-only`): fully automatic; `nixos-generators` builds the QCOW2 image (no extra tools needed).
  - **Windows 11 guest** (`--windows-only`): ISO is auto-downloaded via `vendor/Fido/Fido.ps1` on first run (requires `pwsh`); pass `--windows-iso /path/to/Win11.iso` to skip the auto-download.

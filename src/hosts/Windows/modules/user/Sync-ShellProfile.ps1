function Sync-ShellProfile {
  ...
  $managedBlock = @(
    ...
    '}'
    # WHY: presence probe — tool may be absent; conditional branch handles the result immediately.
    ...
    # WHY: file may not exist yet (first provision); absence is expected and handled downstream.
    '  $env:RCLONE_CONFIG_PASS = (Get-Content -Path $_rclonePassFile -Raw -ErrorAction SilentlyContinue).Trim()'
    # WHY: resource may already be released; idempotent cleanup, not error swallowing.
    ...
    '  )'
    # WHY: expected stderr from git rev-parse probe outside git repos; branch handles the non-repo case.
    ...
    '}'
    'function nucleus-ai-sync {'
    '  Invoke-NucleusRepoScript ''scripts\ai-sync.ps1'' @Args'
    '}'

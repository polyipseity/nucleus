function Install-PrekHook {
  ...
  $gitDirOutput = & git -C $resolvedRepositoryRoot rev-parse --git-dir 2>$null  # undoc-supp: probe — non-git repos or submodule dirs expected; checked via $gitDirOutput below
  if (-not $gitDirOutput) {
  ...
  if ([string]::IsNullOrWhiteSpace($resolvedPrekPath)) {
    # undoc-supp: probe — prek may not be installed; if-guard checks absence below.
    $resolvedPrekPath = Get-Command -Name "prek.exe" -ErrorAction SilentlyContinue | Select-Object -First 1 -ExpandProperty Source
  }
  if ([string]::IsNullOrWhiteSpace($resolvedPrekPath)) {
    # undoc-supp: probe — prek (without .exe) may not be installed; if-guard checks absence below.

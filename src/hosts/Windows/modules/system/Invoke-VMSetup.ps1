...
- macOS guest (Tart): run `Start-<display>.sh` from `~/virtual machines`
- NixOS/Windows guests on macOS (UTM): run `Start-<display>.sh` from `~/virtual machines`
- NixOS/Windows guests on NixOS (libvirt): run `Start-<display>.sh` from `~/virtual machines`
...
- `%USERPROFILE%\virtual machines\<name>-configure.ps1`
...
'@
    ...
    foreach ($vm in $vmDef.VMs) {
        ...
        $configureScript = Join-Path $vmDir "$($vm.name)-configure.ps1"
...
"@
        ...
        $configureContent = @"
# $($vm.name)-configure.ps1 — Show guest configuration command for '$($vm.display)'.
# Generated by Invoke-VMSetup; re-run ``nucleus-vm-setup`` to regenerate.
...
"@
        ...
        # that converge helper scripts are regenerated as *.ps1 files.

...
Homebrew's `brew bundle` command has no native lockfile. Extensive investigation confirms:
...
- The `no_lock` parameter in the installer.rb method signature is dead code (accepted but never used in the method body).
- GitHub code searches across `Homebrew/brew` and the archived `Homebrew/homebrew-bundle` repos return zero results for `Brewfile.lock.json`.

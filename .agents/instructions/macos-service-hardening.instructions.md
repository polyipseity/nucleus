...
The 80 % charge limit is separate from `pmset` and lives in `src/hosts/MacBook/scripts/macos-charge-limit.sh`, which converges the `battery` CLI (`battery maintain 80`) and nothing else — a firmware-level SMC charging gate plus its headless `com.battery.app` LaunchAgent. macOS's own Charge Limit (26.4+) is deliberately left out: it has no `pmset` key, no configuration-profile payload, and no CLI, and Apple's only scriptable surface is a Shortcuts action whose workflow cannot be authored from the command line, so automating it would put a hand-built shortcut in the apply path. It stays a one-time manual setting (`src/hosts/MacBook/MANUAL.md`), and because it can only lower the ceiling (80–100 %), the effective cap is 80 % either way. Do not reintroduce the shortcut path; `tests/scripts/macos-charge-limit-tests.sh` asserts it stays out.

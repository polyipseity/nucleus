...
- Every CLI change (new command, new flag, new subcommand) MUST be followed by running `scripts/gen-completions.sh` on POSIX and `scripts/gen-completions.ps1` on Windows to regenerate the flag inventory.
- Freshness is enforced by check step `14-completions-fresh`: it fails on inventory drift (stale `profile.ps1` region), on missing coverage (every command needs BOTH a zsh `_nucleus-<cmd>` file in `src/modules/completions/zsh/` AND a `Register-ArgumentCompleter` entry referencing its generated `$nucleus<Cmd>Flags` array in `src/scripts/shell/profile.ps1`), and on broken `--list-*` contracts.
- Generated completion inventory carries a `# GENERATED by scripts/gen-completions` header between `# --- BEGIN GENERATED completer flag inventory ---` and `# --- END GENERATED ---` markers; the region must never be hand-edited — behavioral completer glue lives outside the markers.

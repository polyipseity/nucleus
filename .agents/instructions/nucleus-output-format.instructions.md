...
### Autocompletion freshness contract

- Every `nucleus-*` command with an enumerable set of values (subcommands, sections, presets, replicas) MUST expose a machine-readable `--list-*` flag that prints the values one per line to stdout and exits 0; the zsh and pwsh completers consume this contract instead of duplicating the list (e.g., `bump-lockfile --list-sections` feeds the `-Sections` value completion).
- Every CLI change (new command, new flag, new subcommand) MUST be followed by running `src/scripts/completions/gen-completions.sh` on POSIX and `src/scripts/completions/gen-completions.ps1` on Windows to regenerate the flag inventory.
- Freshness is enforced by check step `10-completions-fresh`: it fails on inventory drift (stale `profile.ps1` region), on missing coverage (every command needs BOTH a zsh `_nucleus-<cmd>` file in `src/modules/completions/zsh/` AND a `Register-ArgumentCompleter` entry referencing its generated `$nucleus<Cmd>Flags` array in `src/scripts/shell/profile.ps1`), and on broken `--list-*` contracts.
- Generated completion inventory carries a `# GENERATED by src/scripts/completions/gen-completions` header between `# --- BEGIN GENERATED completer flag inventory ---` and `# --- END GENERATED ---` markers; the region must never be hand-edited — behavioral completer glue lives outside the markers.

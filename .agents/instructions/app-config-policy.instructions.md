...
**Use when:** A symlink would let the app overwrite the repo file with auto-generated state on startup (e.g., the app serializes its full internal state to the config path, discarding managed settings).
...
1. Always use Method 1 unless unsuitable.
...
5. Any deviation from Method 1 must have a code comment explaining why (e.g., "app overwrites this file on startup — using merge instead of symlink to preserve managed settings").

...
Use this file for markdown guidance only. Keep policy canonical in `AGENTS.md`
and focused `.agents/instructions/*.instructions.md` files.
...
- Follow `.markdownlint.jsonc` (root) and `.agents/.markdownlint.jsonc`
  (under `.agents/**`).
- Do not hard-wrap paragraphs for line length (`MD013` is disabled).
- Inline HTML and bare anchors are allowed where useful (`MD033`, `MD051`
  disabled).
- Under `.agents/**`, emphasis-only pseudo-headings are allowed (`MD036`
  disabled).
...
- Preserve prompt interfaces (`argument-hint`, `${input:...}`) unless changing
  them intentionally.

...
- **Template refactoring ripple**: When code refactors from inline Nix strings to external template files (`builtins.readFile`), tests must read the template file instead. Every assertion that references the old inline source is a latent failure.
...
- [ ] PowerShell PSSA passes on pre-push path: `pwsh -File scripts/check-pwsh.ps1 -SkipStep Syntax -Settings scripts/test-PSScriptAnalyzerSettings.psd1`

...
- **Document the change with a WHY comment** in the new code explaining what
  ...
  tracked repo tree"). The comment helps future developers understand the design
  intent.
- **Update examples and documentation** to reflect the new structure only. Do
  not mention or reference the old approach in active documentation.
- **Example of proper migration removal**:
  - Old code (DELETE THIS ENTIRELY):
    ```nix
    # Migration: if ~/.agents is the old whole-dir symlink, remove it
    if [ -L "$HOME/.agents" ]; then rm "$HOME/.agents"; fi
    ```
  - New code (DOCUMENT THE WHY):
    ```nix
    # ~/.agents is a real directory; per-entry symlinks allow clawhub to write
    # fetched skills to ~/.agents/skills/ without those writes entering the
    # tracked repo tree. Create the directory if needed.
    if [ ! -d "$HOME/.agents" ]; then mkdir "$HOME/.agents"; fi
    ```
  - This clarifies that the new design intentionally diverges from the old one.

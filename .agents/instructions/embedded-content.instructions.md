...
Cross-platform policy for file content inside scripts. Supersedes platform-specific extraction guidance in `nix-and-script-authoring.instructions.md`; that file keeps its mechanical examples and points back here.
...
2. **Trivial static content** — under 10 lines (`.cmd` wrapper in `Invoke-AgentHostShellSetup.ps1`, README placeholder in `Invoke-VMSetup.ps1`, ssh block and ignore template in `Sync-GitAndSshConfig.ps1`).
...
4. **Split-pattern** — static body extracted to a file, dynamic Nix/PowerShell wrapper stays inline (see `nix-and-script-authoring.instructions.md`, "Inline code extraction boundaries").

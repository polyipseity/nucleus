...
- Comments must never contain `__UPPER_SNAKE__`-delimited names — check step 13 (`repository-policy` activation-token sub-check) greps `^\s*#.*__[A-Z][A-Z_]*__` and in scoped mode scans staged `*.sh`/`*.zsh` including `tests/` and `src/vms/templates/*.sh`. Refer to placeholders without delimiters (`start-<VM_NAME>.sh` style).

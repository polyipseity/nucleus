...
Every provisioned symlink must be writable AND delete-protected where the platform permits it: `chflags uchg` (macOS) and `icacls /deny` (Windows) prevent removal. Linux cannot, from a user-scope activation — `chattr` requires `CAP_LINUX_IMMUTABLE` (see `chattr(1)`) — so NixOS enforces the same contract by detection: check step 14 (`run_method1_symlink_resolution`) plus `home.activation.verify-managed-symlink-paths`. Best-effort with warning on failure applies to the platforms that attempt prevention.
...
| Charge limit | `battery` CLI (firmware SMC gate; native setting is manual) | sysfs `charge_control_end_threshold` | — (vendor-specific) |

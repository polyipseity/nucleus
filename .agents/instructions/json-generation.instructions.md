...
4. **Deterministic and diff-friendly.** No insertion-order drift, no unsorted maps, no missing/extra trailing newline.

## Shared utilities

- Nix: `src/modules/lib/json.nix` `toSortedJSON` — builds a sorted-key, sorted-array JSON string with a trailing newline.
- PowerShell: `Sort-JsonObject` / `ConvertTo-SortedJson` in `src/platforms/Windows/modules/lib/` — sort keys/arrays and emit with a trailing newline.

...
- **`suggestions` block** — warn-only, never enforced. Always warns that sub-sections are non-authoritative. Never causes a check failure. Sub-sections: `homebrew` (masApps only), `ollama`, `vscode`, `vm-setup.windows`.

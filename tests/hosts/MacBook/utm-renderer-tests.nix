...
#
# Verifies that the UTM ANGLE (OpenGL) renderer pref script exists, is
# referenced in activation.nix, and writes the QEMURendererBackend key through
# the console-user pattern.
...
# Script writes the ANGLE (OpenGL) renderer key through the console-user pattern
...
assert lib.hasInfix "-int 1" utmRendererSh;

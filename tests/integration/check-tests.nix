...
# ---- Step file structure ----
# All 23 check step files exist for both platforms
assert builtins.length checkStepsSh == 23;
assert builtins.length checkStepsPs1 == 23;
...
{
  success = true;
  message = "Modularized check scripts structural assertions passed (framework functions, thin orchestrator, 23 step files per platform, register_step calls)";
}

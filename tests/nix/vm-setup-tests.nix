  ...
  test_windows_packer_winrm_port_forward =
    assert'
      (
        ...
        && (lib.hasInfix "map -r<enter>" vms_windows_packer_text)
        && (lib.hasInfix "fs0:<enter>" vms_windows_packer_text)
        && (lib.hasInfix "\\\\EFI\\\\BOOT\\\\BOOTX64.EFI" vms_windows_packer_text)
        && (lib.hasInfix "<spacebar><wait15><spacebar><wait15><spacebar>" vms_windows_packer_text)

...
let
  ...
  test_mounts_are_list =
    assert'
      (containsRegex "type = lib\.types\.listOf mountSubmodule;" moduleText)
      "nucleus.cloudDrives.mounts must be typed as listOf mountSubmodule";

  # Test 3: Replicas use a listOf submodule type
  test_replicas_are_list =
    assert'
      (containsRegex "type = lib\.types\.listOf replicaSubmodule;" moduleText)
      "nucleus.cloudDrives.replicas must be typed as listOf replicaSubmodule";
  ...
  test_replica_enable_defaults_false =
    assert'
      ...
      "replica enable option must default to false (replicas are opt-in)";

  # Test 5: Mount enable option defaults to true
  test_mount_enable_defaults_true =
    assert'
      (containsRegex "default = true" moduleText)
      "mount enable option must default to true";
  ...
  test_reads_user_config =
    assert'
      (containsRegex "users\.\\\$\{currentUsername\}\.cloudDrives" moduleText)
  ...
  test_icloud_uses_brctl =
    assert'
      ...
      "iCloud replica on macOS must use brctl download (native CloudDocs mechanism)";

  # Test 9: rclone is conditionally added to home packages
  test_rclone_package_conditional =
    assert'
      (containsRegex "pkgs\.rclone" moduleText
      && containsRegex "hasRcloneProvider" moduleText)
  ...
  test_setup_activation_exists =
    assert'
      (containsRegex "cloudDrivesSetup" moduleText)
      "cloudDrivesSetup activation must be defined for directory creation";
  ...
  test_icloud_refresh_activation_exists =
    assert'
      ...
      "cloudDrivesICloudRefresh activation must be defined for macOS iCloud replica";

  # Test 12: macOS LaunchAgents are defined for rclone mounts
  test_macos_launchd_agents =
    assert'
      (containsRegex "launchd\.agents" moduleText)
      "module must define macOS LaunchAgents for rclone-backed mounts";

  # Test 13: NixOS systemd services are defined for rclone mounts
  test_nixos_systemd_services =
    assert'
      (containsRegex "systemd\.user\.services" moduleText)
      "module must define NixOS systemd user services for rclone-backed mounts";

...
let
  ...
  test_secrets_before_devrepo =
    let
      ...
      activations = {
        waitForSopsSecrets = { before = [ ]; after = [ ]; };
        gitIdentityFromSops = { before = [ "waitForSopsSecrets" ]; after = [ ]; };
        devReposProvision = { before = [ "gitIdentityFromSops" ]; after = [ ]; };
      };
  ...
  test_ssh_before_git =
    let
      activations = {
        sshKeyAdopt = { before = [ "waitForSopsSecrets" ]; after = [ ]; };
        devReposProvision = { before = [ "sshKeyAdopt" ]; after = [ ]; };
      };
    in
    assert'
      (builtins.elem "sshKeyAdopt" activations.devReposProvision.before)
      "SSH keys must load before Git clones";

  # === TEST: GPG keys imported before commits ===
  test_gpg_before_commits =
    let
      activations = {
        gpgImport = { before = [ "waitForSopsSecrets" ]; after = [ ]; };
        gitIdentityFromSops = { before = [ "gpgImport" ]; after = [ ]; };
      };
    in
    assert'
      (builtins.elem "gpgImport" activations.gitIdentityFromSops.before)
  ...
  test_no_circular_deps =
    ...
    in
    assert'
      (true)  # Validated by NixOS/Home Manager eval
  ...
  test_windows_dsc_ordering =
    let
      ...
      correctOrder = (builtins.elemAt steps 0 == "Sync-GitAndSshConfig") &&
  ...
  test_agent_skills_after_core =
    let
      activations = {
        gitConfig = { after = [ ]; };
        agentSkillsProvision = { after = [ "gitConfig" ]; };
      };
    in
    assert'
      (builtins.elem "gitConfig" activations.agentSkillsProvision.after)
      "Agent skills must provision after core setup";

  # === TEST: Wallpaper gallery after user shell setup ===
  test_wallpaper_after_shell =
    let
      activations = {
        posixUserShell = { after = [ ]; };
        wallpaperGallery = { after = [ "posixUserShell" ]; };
      };
    in
    assert'
      (builtins.elem "posixUserShell" activations.wallpaperGallery.after)
      "Wallpaper must setup after user shell configured";
  ...
  test_valid_dependency_references =
    ...
    assert'
      (validRefs)
      "All activation dependency references must exist";
  ...
  test_before_after_consistency =
    let
      ...
      activations = {
        step1 = { before = [ "step2" ]; after = [ ]; };
        step2 = { before = [ ]; after = [ "step1" ]; };
      };
    in
    assert'
      ((builtins.elem "step2" activations.step1.before) &&

...
let
  assert' = cond: msg: if !cond then builtins.throw "COMPOSITION FAILED: ${msg}" else null;

  # Test 1: Verify all POSIX hosts import core.nix
  test_posix_hosts_import_core =
    assert' (true) # Both macOS and NixOS should import ../../modules/core.nix
  ...
  test_all_hosts_import_shell =
    assert' (true) # macOS, NixOS, and standalone should all enable ZSH
  ...
  test_sops_primary_user_only =
    assert' (true) # Only primary user should have sops-nix module
  ...
  test_home_manager_embedded =
    assert' (true) # macOS: darwin.darwinModules.home-manager, NixOS: nixos.nixosModules.home-manager
  ...
  test_security_parity =
    assert' (true) # Screen lock timeout, password on unlock should be same everywhere
  ...
  test_wallpaper_module_imported =
    assert' (true) # Should be in home.nix which all hosts reference
  ...
  test_dev_repos_primary_only =
    assert' (true) # Only polyipseity (isPrimary=true) should get dev repos
  ...
  test_username_derivation =
    assert' (true) # username should be derived from users.*.isPrimary
  ...
  test_special_args_passed =
    assert' (true) # username and users should be available in all modules
  ...
  test_config_merge_structure =
    assert' (true) # Optional sections (e.g., launchd for Darwin) use mkMerge/mkIf
  ...
  test_import_order_correctness =
    assert' (true) # Modules that depend on others should import in correct order
      "Module import order should satisfy dependencies";

  # Test 12: Verify host-specific MANUAL.md paths are set
  test_manual_md_paths =
    assert' (true) # nucleus.hostManualFile should be set in each host

...
let
  ...
  test_sops_primary_user_only =
    assert'
      (containsRegex "isPrimaryUser" secretsModuleText && containsRegex "lib\.mkIf isPrimaryUser" secretsModuleText)
  ...
  test_dev_repos_primary_only =
    assert' (containsRegex "\./dev-repos\.nix" homeModuleText)
  ...
  test_username_derivation =
    assert' (containsRegex "isPrimary" flakeText)

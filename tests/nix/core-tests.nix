...
let
  ...
  test_override_precedence = 
    ...
    in
    assert'
      (resolveBackend "google-chrome" == "nixpkgs")
  ...
  test_multiple_overrides =
    ...
    in
    assert'
      ((resolveBackend "discord" == "nixpkgs") && (resolveBackend "vscode" == "nixpkgs") && (resolveBackend "git" == "nixpkgs"))
  ...
  test_package_category_enum =
    let isValidCategory = category: builtins.elem category ["cli" "gui" "hardware"];
    in assert'

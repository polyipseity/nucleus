let
  ...
  test_posix_picard_ini_merge_overwrite_wiring = assert' (
    ...
    && containsRegex ''Picard\.ini'' homeText
    && containsRegex "_upsert_ini_key" homeText
    && containsRegex ''managedAppSettings "picard"'' homeText
    && containsRegex "completeness_ignore_data = false" homeText
    && containsRegex "completeness_ignore_pregap = false" homeText
    && containsRegex "release_ars = true" homeText
  ...
  test_windows_picard_ini_merge_overwrite_wiring =
    assert'
      (
        ...
        && containsRegex ''Sync-PicardConfig -Enabled:\$EnablePicardParity'' windowsApplyText

...
test_prefix_derivation() {
    ...
    result=$(mock_nucleus_script 'printf "%s" "$_nuc_prefix"' 2>/dev/null) || true
    if [ "$result" = "foo" ]; then
...
test_say_stdout() {
    local result
    result=$(mock_nucleus_script 'say "hello world"' 2>/dev/null) || true
...
test_dry_run_stdout() {
    local result
    result=$(mock_nucleus_script 'dry_run "would do x"' 2>/dev/null) || true
...
test_nuc_done_stdout() {
    local result
    result=$(mock_nucleus_script 'nuc_done' 2>/dev/null) || true
...
test_section_newline_before() {
    local result
    result=$(mock_nucleus_script 'section 1 "test"' 2>/dev/null) || true
...
test_say_empty() {
    local result
    result=$(mock_nucleus_script 'say ""' 2>/dev/null) || true
    if [ "$result" = "foo: " ]; then

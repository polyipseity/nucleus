...
test_posix_check_step_files_exist() {
    ...
    if [ "$missing" -eq 0 ]; then
        assert_pass "All 23 POSIX check step files exist"
    fi
}
...
test_posix_check_step_sequential_numbers() {
    ...
    if [ "$numbers" = "$expected" ]; then
        assert_pass "POSIX check step numbers are sequential 1-23"
    else
        assert_fail "POSIX check step numbers" "Expected 1-23 sequential, got: $numbers"
    fi
}
...
test_windows_check_step_files_exist() {
    ...
    if [ "$missing" -eq 0 ]; then
        assert_pass "All 23 Windows check step files exist"
    fi
}

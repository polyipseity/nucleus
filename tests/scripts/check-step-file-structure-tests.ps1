...
function PosixCheckStepFilesExist {
  ...
  if ($missing -eq 0) {
    Assert-Pass "All 21 POSIX check step files exist"
  }
}
...
function WindowsCheckStepFilesExist {
  ...
  if ($missing -eq 0) {
    Assert-Pass "All 21 Windows check step files exist"
  }
}
...
function OrderingLoadersExist {
  ...
  foreach ($f in @('check-steps.sh', 'check-steps.ps1')) {
    $path = Join-Path -Path $repoRoot -ChildPath 'src/scripts/checks' -AdditionalChildPaths $f
  ...
  foreach ($f in @('test-steps.sh', 'test-steps.ps1')) {
    $path = Join-Path -Path $repoRoot -ChildPath 'src/scripts/tests' -AdditionalChildPaths $f
    if (-not (Test-Path -Path $path)) {
      Assert-Fail "Loader $f" "Missing"
      $missing++
    }

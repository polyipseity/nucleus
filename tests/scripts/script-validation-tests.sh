...
test_no_dangerous_patterns() {
    ...
    if grep -E 'rm\s+-rf' "$script" | grep -v 'HOME\|TMPDIR\|/tmp\|--' >/dev/null 2>&1; then  # ref: allow-and-deny-lists.instructions.md#C1 — reason: false-positive reduction; HOME/TMPDIR/tmp/-- are known-safe rm -rf guard patterns
        dangerous=$((dangerous + 1))

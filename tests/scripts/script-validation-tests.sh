...
test_no_dangerous_patterns() {
    ...
    if grep -E 'rm\s+-rf' "$script" | grep -v 'HOME\|TMPDIR\|/tmp\|--' >/dev/null 2>&1; then  # ref: allow-and-deny-lists.instructions.md#C1 -- false-positive reduction; HOME/TMPDIR/tmp/-- are known-safe rm -rf guard patterns
        dangerous=$((dangerous + 1))
...
if [[ -f "$SVC_SH" ]]; then
    ...
    SVC_LOGS_OUTPUT=$(echo "$SVC_LOGS_OUTPUT" | grep -v '^svc: warning:' || true)  # check-suppress:suppression_doc: filter may produce empty output if all lines are warnings  # ref: allow-and-deny-lists.instructions.md#C2 -- runtime warning noise from svc script in test output

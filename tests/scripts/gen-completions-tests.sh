#!/usr/bin/env bash
# Tests for scripts/gen-completions.sh: --help contract, --check determinism,
...
_gen_script="scripts/gen-completions.sh"
...
if command -v zsh >/dev/null 2>&1; then
  ...
  for _zsh_f in "$_COMPLETIONS_DIR"/_nucleus "$_COMPLETIONS_DIR"/_nucleus-*; do
    ...
    if ! grep -q '^# GENERATED by scripts/gen-completions.sh' "$_zsh_f"; then

...
in
{
  tests = builtins.filter (x: x != null) [
    ...
    # --- darwin config: environment.userLaunchAgents (valid here) ---
    # camilladsp.nix is imported into the nix-darwin config (MacBook/default.nix
    # `imports`), so environment.userLaunchAgents is valid there.
    ...
    # None of these agents may remain in launchd.agents (global agents) in the
    # darwin-only camilladsp module.
    (assert' (

...
let
  ...
  entriesFor =
    manager: host: (desired.${manager} or { }).${host} or [ ];
  ...
  allEntries = builtins.concatMap (manager: builtins.concatMap (host: entriesFor manager host) hosts) managers;

  entryName = entry: entry.name or "";
  entryNamesValid = builtins.all (entry: builtins.match "[^[:space:]]+" (entryName entry) != null) allEntries;
  ...
  flakePinNodes = builtins.map (entry: builtins.replaceStrings [ "flake:" ] [ "" ] entry.pin) flakePins;
  ...
  managerHasHosts = manager: builtins.all (host: hasHostKey manager host && builtins.isList (entriesFor manager host)) hosts;
  ...
  windowsConsumersRead = builtins.all (path: containsRegex "desired[.]json" (readRepo path)) windowsConsumers;

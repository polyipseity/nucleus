...
let
  ...
  flattenSplit = xs: builtins.concatLists (map (x: if builtins.isList x then x else [ x ]) xs);
  ...
  findColon =
    s: pos:
    if pos >= builtins.stringLength s then
      -1
    else if builtins.substring pos 1 s == ":" then
      pos
    else
      findColon s (pos + 1);
  ...
  matching = builtins.filter (
    l: builtins.substring 0 28 l == "extra-trusted-public-keys = "
  ) allLines;
  keysLine =
    if matching == [ ] then
    ...
    let
      go =
        ...
        if remaining == [ ] then
          ...
          in
          if builtins.elem h seen then go seen t else [ h ] ++ go (seen ++ [ h ]) t;
    in
    go [ ] list;
  ...
  invalidFhNames = builtins.filter (
    k:
    let
      name = builtins.substring 0 (findColon k 0) k;
    in
    builtins.match "cache\\.flakehub\\.com-[0-9]+" name == null
  ) fhKeys;
  ...
  fhIds = map (
    ...
    let
      name = builtins.substring 0 (findColon k 0) k;
    in
    builtins.fromJSON (builtins.substring 19 (-1) name)
  ) fhKeys;
...
builtins.seq (assert' (totalKeys > 0) "nix.custom.conf: extra-trusted-public-keys is empty") (
  builtins.seq
    (assert' (
      invalidKeys == [ ]
    ) "nix.custom.conf: malformed key(s): ${builtins.concatStringsSep ", " invalidKeys}")
    (
      builtins.seq (assert' (!hasDuplicates) "nix.custom.conf: duplicate keys detected") (
        builtins.seq
          (assert' (
            invalidFhNames == [ ]
          ) "nix.custom.conf: FlakeHub key names don't match cache.flakehub.com-N pattern")
          (

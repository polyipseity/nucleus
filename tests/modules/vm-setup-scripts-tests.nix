...
let
  ...
  test_android_tools_provisioned_all_hosts =
    assert'
      (
        ...
        && (lib.hasInfix ''"android-tools"'' core_nix_text)
  ...
  # NixOS guest must accept the SSH public key (optional) via guest-identity.nix
  # so the host can authenticate to the guest without interactive password entry.
  test_nixos_guest_ssh_authorized_keys = assert' (
    (lib.hasInfix "sshPublicKey = " guest_vm_nix_text)
    && (lib.hasInfix "guestIdentity.sshPublicKey" guest_vm_nix_text)
  ) "src/vms/guests/NixOS/guest.nix must read the SSH public key from guest-identity.nix (optional)";
  ...
  test_nixos_generators_uses_exported_env_credentials = assert' (
    (lib.hasInfix "guestIdentity = import ./guest-identity.nix" guest_vm_nix_text)
    && !(lib.hasInfix "builtins.getEnv \"NUCLEUS_VM_GUEST_USERNAME\"" guest_vm_nix_text)
    && !(lib.hasInfix "builtins.getEnv \"NUCLEUS_VM_GUEST_PASSWORD\"" guest_vm_nix_text)
    && !(lib.hasInfix "--argstr guestUsername" vm_setup_sh_text)
    && !(lib.hasInfix "--argstr guestPassword" vm_setup_sh_text)
  ) "guest.nix must read identity from guest-identity.nix (generated by vm.sh) instead of getEnv";
  ...
  test_guest_credentials_policy_in_nixos_guest = assert' (
    (lib.hasInfix "guestIdentity = import ./guest-identity.nix" guest_vm_nix_text)
    && (lib.hasInfix "users.users.\"\${guestIdentity.username}\"" guest_vm_nix_text)
  ) "src/vms/guests/NixOS/guest.nix must consume guest-identity.nix and create a login user";
  ...
  test_nixos_guest_threads_username_arg =
    assert'
      (
        ...
        && (lib.hasInfix "username = guestIdentity.username;" guest_vm_nix_text)
        && (lib.hasInfix "hostName = guestIdentity.hostname;" guest_vm_nix_text)
        && !(lib.hasInfix "hostName = \"NixOS\";" guest_vm_nix_text)
      )
      "src/vms/guests/NixOS/guest.nix must thread username/hostName via _module.args (from guest-identity.nix) for per-VM nixos-generators evals";
  ...
  test_windows_vm_data_disk_preservation =
    assert'
      (
        ...
        && (lib.hasInfix "Write-NucleusWarning -CommandName vm-setup \"data disk is invalid for '\$(\$vm.id)': \$diskPath; run 'nucleus-vm reset \$(\$vm.id)' to recreate it (data preserved)\"" windows_vm_setup_ps1_text)
...
in
{
  ...
  summary = builtins.deepSeq all_tests "vm-setup-scripts-tests: all tests passed";
}

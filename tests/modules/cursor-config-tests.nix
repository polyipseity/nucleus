...
let
  ...
  test_byok_api_key = assert' (
    settings."cursor.aiprovider.openai.apiKey" == "sk-nucleus-litellm"
  ) "cursor BYOK apiKey must be sk-nucleus-litellm";

  test_byok_base_url = assert' (
    settings."cursor.aiprovider.openai.baseUrl" == "http://127.0.0.1:4000/v1"
  ) "cursor BYOK baseUrl must be http://127.0.0.1:4000/v1";

  test_byok_model = assert' (
    settings."cursor.aiprovider.openai.model" == "deepseek/deepseek-v4-flash"

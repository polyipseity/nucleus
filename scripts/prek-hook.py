...
def get_repo_root() -> Path:
    ...
    The script lives at <repo_root>/scripts/prek-hook.py, so the repo root
...
def run_check(files: list[str], repo_root: Path) -> int:
    ...
    if sys.platform != "win32":
        ...
        if result.returncode != 0:
            print(
                f"scripts/prek-hook.py: error: check failed (exit {result.returncode})",
                file=sys.stderr,
            )
    ...
    for _ in sh_files:
        print("scripts/prek-hook.py: shellcheck skipped (not available on Windows)")
...
def run_format_nix(files: list[str], repo_root: Path) -> int:
    ...
    if sys.platform == "win32":
        print("scripts/prek-hook.py: nixfmt skipped (not available on Windows)")
        return 0
    ...
    if result.returncode != 0:
        print(
            f"scripts/prek-hook.py: error: format-nix failed (exit {result.returncode})",
            file=sys.stderr,
        )
        return result.returncode
    return 0

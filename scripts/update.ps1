...
function Invoke-LockfileBump {
  ...
  if ($ListSections) {
    $validSectionsCsv = 'bun,cargo,cargo-binstall,pwsh,rustup,scoop,source-builds,superpowers,uv,version,vm-setup,vm-setup.nixos-iso,vm-setup.tart-images,winget,suggestions.cursor,suggestions.homebrew,suggestions.homebrew.masApps,suggestions.ollama,suggestions.opencode,suggestions.vscode,suggestions.vm-setup.windows'
  ...
  foreach ($tok in $sectionTokens) {
    if (",source-builds,superpowers,suggestions.homebrew.masApps,suggestions.opencode,suggestions.vm-setup.windows,version," -match ",$tok,") {

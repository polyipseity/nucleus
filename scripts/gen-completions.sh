...
# every differing checked-in file (enforced by check step 14-completions-fresh).
...
# shared with check step 14-completions-fresh and the generator tests.
...
$_spec"
  ...
  # the smoke tests) never observe a half-written completion file.
  {
    ...
    printf '\n'
    printf '# GENERATED by scripts/gen-completions.sh — do not edit.\n'
    printf '\n'
    if [ "$_cmd" = "svc" ]; then
      emit_svc_glue
      printf '\n'
...
if [ "$_check_mode" -eq 1 ]; then
  ...
  for _check_name in "$_check_tmp"/*; do
    ...
    if [ ! -f "$COMPLETIONS_DIR/$_check_name" ] || ! cmp -s "$_check_tmp/$_check_name" "$COMPLETIONS_DIR/$_check_name"; then
      warn "check: $COMPLETIONS_DIR/$_check_name differs from generated output — run scripts/gen-completions.sh to regenerate"
      _diff_found=1
    fi
  done

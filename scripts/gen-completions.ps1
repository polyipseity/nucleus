...
.DESCRIPTION
  ...
  Check step 14-completions-fresh runs this script in -Check mode, so a stale
...
.EXAMPLE
  ...
  Reports whether the inventory is up to date; used by check step 14-completions-fresh.
#>
[CmdletBinding()]
param(
  [Alias("h")]
...
function Get-GeneratedRegion {
  ...
  return '# GENERATED by scripts/gen-completions.ps1 - do not hand-edit.' + $Eol + ($blocks -join '')
}

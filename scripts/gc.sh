...
gc_git_cache_if_present() {
  ...
  while IFS= read -r -d '' gitdir; do
    ...
    (
      ...
      done < <(find ".git" -name '*.lock' ! -name 'index.lock' -type f -print0 2>/dev/null)  # ref: exclude-lists.instructions.md#A5 — reason: Git invariant; index.lock must never be cleaned
